...
I've also used the librosa library to plot the power spectrogram of the Calibration Circle's audio (which I've included in the repo) which, though only processing the signal's harmonic magnitudes over <i>time<\i>, was able to produce an image of a semicircle (which "echo" across the spectrum). I'm not versed in DSP but I would imagine processes like this would produce a more accurate image.
